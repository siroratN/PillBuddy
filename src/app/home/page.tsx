...
const NotificationsPage = () => {
	...
	useEffect(() => {
		const fetchNotification = async () => {
			try {
				const response = await axios.get(`/api/notifications/time`);
	...
	return (
		<div>
			<h1 className="text-center text-3xl mt-3">สวัสดี, {user?.firstName || 'ผู้ใช้'}</h1>
			...
			{notification ? (
				<div
					...
					onClick={() => router.push(`/notification/${notification.closestNotification.id}`)}
				>
					...
					{notification.allMedicines.map((pill) => {
						...
						return (
							<li key={pill.medicines.name} className="text-lg">
								{pill.medicines.name} {pill.notification_medicines.dosage_amount || 1}{' '}
								{pill.notification_medicines.dosage_amount == 1 ? 'pill' : 'pills'}{' '}
