...
const MedicineList = ({ medicine }: { medicine: MedicineNotification }) => {
	...
	const doneMedicine = async () => {
		await axios
			.patch(`/api/notifications/medicine`, {
				data: {
					medicineInNotiId: medicine.medicineInNotiId,
				},
			})
			.then((data) => window.location.reload());
	};
	...
	return (
		<div className="rounded-xl shadow-md border border-1 border-neutral-200 rounded ">
			{/* {medicine.success+""} */}
			<div
				// medicine.success ? '#99DDCC' : '#FFB6B9'
				className={`flex flex-col gap-4 border-l-[14px] p-4 ${
					medicine.success ? 'border-l-[#99DDCC]' : 'border-l-[#FFB6B9]'
				} rounded-lg`}
			>
				...
				<div className="flex gap-2 self-center">
					<Button onClick={doneMedicine}>Done</Button>
