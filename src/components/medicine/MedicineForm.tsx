...
const MedicineForm = ({ notificationId }: { notificationId: string }) => {
	const scheduleSchema = z.object({
		notification_id: z.coerce.number(),
		medicine_id: z.coerce.number(),
		dosage_amount: z.coerce.number().int().gt(0),
		timing: z.enum(['before meals', 'after meals', 'with meals', 'between meals']),
	});
			...
			<Form {...form}>
				<form action="" onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel className="text-lg">Timing</FormLabel>
								<FormControl>
